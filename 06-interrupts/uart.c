...
/*
 * Interpretation of the two bits of the LINE STATUS REGISTER (LSR):
 * LSR BIT 0:
...
int uart_getc(void)
{
	if (LSR_RX_READY & uart_read_reg(LSR)){
...
/* 
 * handle a uart interrupt, raised because input has arrived, called form trap.c 
 */
void uart_isr(void)
{
	while(1){
